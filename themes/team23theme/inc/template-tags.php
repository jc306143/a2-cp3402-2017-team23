...
function team23theme_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'team23theme' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'team23theme' ),
...
function team23theme_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && team23theme_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'team23theme' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
